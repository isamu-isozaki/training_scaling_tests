...
def plot_scaling(
    ...
    **kwargs: kwargs given to plt.figure()
    ...
    for name, group in df.groupby(df.index):
        ...
        baseline = group.imagespersec.values[0]
        df.loc[name, "baseline"] = baseline
    df["scaling"] = df[imagespersec_col] / df["baseline"]
    df["efficiency"] = df[imagespersec_col] / ( (df[node_col]/min_node) * df["baseline"])
    ...
    for i, (name, group) in enumerate(df.groupby(df.index)):
        ...
        if plot_scaling:
            ...
            imagespersec = group[imagespersec_col].values
